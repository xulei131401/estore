...
<div class="pd-20">
  ...
  <div class="cl pd-5 bg-1 bk-gray mt-20">
    <span class="l">
    <a href="javascript:;" onClick="user_add('550','','添加用户','<?php echo U('Home/User/userAdd');?>')" class="btn btn-primary radius"><i class="icon-plus"></i> 添加用户</a></span>
  ...
  <table class="table table-border table-bordered table-hover table-bg table-sort">
    ...
    <tbody>
    <?php if(is_array($userList)): foreach($userList as $k=>$user): ?><tr class="text-c">
        <td><input type="checkbox" value="1" name=""></td>
        <td><?php echo ($user["id"]); ?></td>
       
        <td><u style="cursor:pointer" class="text-primary" onclick="user_show('10001','360','','张三','user-show.html')"><?php echo ($user["account"]); ?></u></td>
        <?php if($user["sex"] == 0): ?><td>男</td>
        <?php else: ?>
         <td>女</td><?php endif; ?>
        <td><img src="<?php echo ($user["pic"]); ?>" alt="头像" width="125" height="100"></td>
        <td><?php echo ($user["email"]); ?></td>
        <td class="text-l"><?php echo ($user["birthday"]); ?></td>
        <td><?php echo ($user["identity"]); ?></td>
        <td class="user-status"><span class="label label-success">已启用</span></td>
        <td class="f-14 user-manage"><a title="编辑"  href="<?php echo U('Home/User/userEdit',array('id'=>$user['id']));?>" class="ml-5" style="text-decoration:none"><i class="icon-edit"></i></a> <a style="text-decoration:none" class="ml-5" href="<?php echo U('Home/User/editPass',array('id'=>$user['id']));?>" title="修改密码"><i class="icon-key"></i></a> <a title="删除" href="<?php echo U('Home/User/userDelete',array('id'=>$user['id']));?>"  class="ml-5" style="text-decoration:none"><i class="icon-trash"></i></a></td>
